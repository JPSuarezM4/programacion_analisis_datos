...
class DatabaseMonitor:
    def __init__(self):
        self.rutadb = "src/edu_pad/static/db/sol_analisis.db"
        self.tabla = "sol_analisis"
    ...
    def ejecutar_monitoreo(self):
        ...
        if not self.verificar_base_datos():
            self.enviar_alerta(
                ...
                f"La base de datos sol_analisis.db no se encuentra o no es accesible. Fecha: {datetime.now()}"
            )
            return False
